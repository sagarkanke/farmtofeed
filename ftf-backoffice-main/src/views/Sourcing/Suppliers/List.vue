...
const handleMenuAction = (event, item) => {
    ...
    if (event === 'Edit') {
        store.commit('suppliers/setSelectedItem', selectedItemToDelete);
        router.push({ path: '/sourcing/edit-supplier' });

    } else if (event === 'Delete') {
        deleteProductDialog.value = true;
    } else if (event === 'View') {
        store.commit('suppliers/setSelectedItem', selectedItemToDelete);
        router.push({ path: '/sourcing/supplier-details' });
    }
...
const newOrder = () => {
    router.push({ path: '/sourcing/new-suppliers' });
};
...
<template>
    <div class="grid" :style="{ 'margin-left': '-50px', 'margin-top': '-30px' }">
        ...
        <div class="col-12">
            <div class="card">
                <DataTable ref="dt" :value="supplierList" v-model:selection="selectedOrders" dataKey="id" :loading="isLoading"
                    ...
                    <template #header>
                        <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center"
                            ...
                            <h5 class="m-0 text-color-secondary">Suppliers</h5>
                        </div>
                        <div class="flex justify-content-between gap-6 mt-2" :style="{ 'margin-left': '-15px' }">
                            ...
                            <div class="flex gap-2" :style="{ 'margin-right': '-22px' }">
                                ...
                                <Button type="button" label="Add Supplier" icon="pi pi-plus"
                    ...
                    <Column field="supplier" header="Supplier" headerStyle="width:14%; min-width:13rem;">
                        <template #body="slotProps">
                            <span :style="{ 'color': '#747C50' }">{{ slotProps.data?.user.name || '-' }}</span>

                        </template>
                    </Column>
                    <Column field="contact" header="Contact" headerStyle="width:14%; min-width:11rem;">
                        <template #body="slotProps">
                            {{ slotProps.data?.contact || '-' }}

                        </template>
                    </Column>
                    <Column field="phoneNo" header="Phone Number" headerStyle="width:14%; min-width:13rem;">
                        <template #body="slotProps">
                    ...
                    <Column field="email" header="Email" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            {{ slotProps.data?.user?.email || '-' }} </template>
                    </Column>
                    <Column field="type" header="Type" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            {{ slotProps.data?.supplier_type || '-' }} </template>
                    </Column>
                    <Column field="cropsAvailable" header="Crops Available" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            {{ slotProps.data?.products?.length || 0 }} </template>
                    </Column>
                    <Column field="ordersReceived" header="Orders Received" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            {{ slotProps.data?.orders_received || 0 }} </template>
                    </Column>
                    <Column field="paymentsDone" header="Payments Done" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            {{ slotProps.data?.payments_done || '-' }} </template>
                    </Column>
                    ...
                    <Column field="lastContact" header="Last Contact" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            {{ formatDate(slotProps.data?.updated_at) || '-' }} </template>
                    </Column>
    ...
    <Sidebar v-model:visible="viewBillingDetails" :baseZIndex="1000" position="right" :style="{ width: '400px' }">
        <template #header>
            <div class="p-d-flex p-flex-column p-ai-start">
                ...
                <Badge :value="selectedItem?.status" :class="['p-mt-2 p-border-round', getBadgeSeverity('active')]"
                    :style="{
