...
const vehicles = computed(() => store.getters['mileage/data']);
const isLoading = computed(() => store.getters['mileage/loading']);
const error = computed(() => store.getters['mileage/error']);
...
const onRowEditInit = (event) => {
    editingRows.value[event.index] = true;
};

const onRowEditSave = (event) => {
    const index = vehicles.value.findIndex(vehicle => vehicle.id === event.data.id);
    vehicles.value[index] = { ...event.data }; // Update the data with edited values
    editingRows.value[event.index] = false;
};
    ...
    <div class="card" :style="{ marginLeft: '-26px' }">
        ...
        <div>
            ...
            <DataTable :value="vehicles" :loading="isLoading" :rows="5" :paginator="true" responsiveLayout="scroll"
                editMode="row" dataKey="id" @row-edit-init="onRowEditInit" @row-edit-save="onRowEditSave"
                ...
                <Column field="first_mile" header="First Mile" :sortable="false">
                    <template #body="slotProps">
                ...
                <Column field="last_mile" header="Last Mile" :sortable="false">
                    <template #body="slotProps">
                ...
                <Column field="distance_covered" header="Distance covered" :sortable="false">
                    <template #body="slotProps">
                        {{ slotProps.data.distance_covered }}
