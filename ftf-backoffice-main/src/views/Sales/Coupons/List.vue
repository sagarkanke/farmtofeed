...
<template>
    <div class="grid" :style="{ 'margin-left': '-50px', 'margin-top': '-30px' }">
        ...
        <div class="col-12">
            <div class="card">
                <DataTable ref="dt" :value="filteredOrders" dataKey="id"
                        ...
                        <div class="flex justify-content-between gap-6 mt-2" :style="{ 'margin-left': '-15px' }">
                            <div class="flex gap-2">
                                ...
                                <div>
                                    ...
                                    <Dropdown id="state" v-model="status" :options="statuses" optionLabel="value" @change="fetchCouponList()"
                            ...
                            <div class="flex gap-2" :style="{ 'margin-right': '-22px' }">
                                ...
                                <Button type="button" label="New Coupon" icon="pi pi-plus"
                    ...
                    <Column field="created" header="Created" :sortable="true" headerStyle="width:14%; min-width:10rem;">
                    ...
                    <Column field="amount" header="Amount" :sortable="true" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            ...
                            {{ slotProps.data.discount_value }} </template>
                    </Column>
                    <Column field="usage" header="Usage" :sortable="true" headerStyle="width:14%; min-width:8rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">Usage</span>
                            {{ slotProps.data.total_usage_limit }}
                        </template>
                    </Column>
                    ...
                    <Column field="endDate" header="End Date" :sortable="true"
                        ...
                        <template #body="slotProps">
                            ...
                            {{ slotProps.data.end_date || '-' }} </template>
                    </Column>
                ...
                <Dialog v-model:visible="addFlashSale" :style="{ width: '450px' }" :modal="true" class="p-fluid">
                    ...
                    <div class="flex justify-content-between gap-2">
                        <div>
                            <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Coupon
                                Code</label>
                            <InputText id="username2" type="username" class="p-error mt-1" v-model="couponCode"
                                aria-describedby="username-help" />
                        </div>
                        <div :style="{ 'align-self': 'end' }">
                            <Button type="button" label="Generate Coupon Code" :style="{
                                ...
                                disabled: 'couponCode',
                                width: 'max-content',
                            }" @click="generateCouponCode()"></Button>
                        </div>
                    </div>
                    <div class="field mt-3">
                        <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Coupon Type</label>
                        <Dropdown v-model="selectedCouponType" :options="couponTypes" optionLabel="name"
                            optionValue="id" placeholder="Select Coupon">
                        </Dropdown>
                    </div>
                    <div class="field">
                        <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Customer</label>
                        <Dropdown v-model="selectedCustomer" :options="customerList" optionLabel="name"
                            placeholder="Select Customer">
                        </Dropdown>
                    </div>
                    <div class="flex justify-content-between gap-2">
                        <div>
                            <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Discount
                                Type</label>
                            <Dropdown v-model="selectedDiscountType" :options="discountTypes" optionLabel="name"
                                optionValue="label" placeholder="Percentage(%)" class="mt-1">
                            </Dropdown>
                        </div>
                        <div>
                            <label for="description" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Coupon
                                Amount</label>
                            <InputNumber class="mt-1" id="price" v-model="couponAmount" :required="true" />
                        </div>
                    </div>
                    <div class="field mt-3">
                        <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">User Limit</label>
                    ...
                    <div class="flex justify-content-between mt-3 gap-2">
                        <div>
                            <label for="orderDate" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Start
                                Date</label>
                            <Calendar class="mt-1" v-model="selectedStartDate" :style="{ borderRadius: '8px' }"
                                :showIcon="true" :manualInput="false" placeholder="DD/MM/YYYY"></Calendar>
                        </div>
                        <div>
                            <label for="orderDate" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Expiry
                                Date</label>
                            <Calendar class="mt-1" v-model="selectedEndDate" :style="{ borderRadius: '8px' }"
                                :showIcon="true" :manualInput="false" placeholder="DD/MM/YYYY"></Calendar>
                        </div>
                    </div>
                    <template #footer>
                        ...
                        <Button type="button" label="Add Coupon" icon="pi pi-plus"
                ...
                <Dialog v-model:visible="editFlashSaleDialog" :style="{ width: '450px' }" :modal="true" class="p-fluid">
                    <template #header>
                        <div class="p-d-flex p-ai-center">
                            ...
                            <span class="p-ml-n6 ml-2"
                                style="color: darkgreen; font-weight: 700; font-size: larger">Edit Coupon</span>
                            <!-- PrimeFlex classes for margin-left and bold text -->
                        </div>
                    </template>
                    <div class="flex justify-content-between gap-2">
                    ...
                    <div class="flex justify-content-between gap-2">
                        ...
                        <div>
                            <label for="description" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Coupon
                                Amount</label>
                            <InputNumber class="mt-1" id="price" v-model="couponAmount" />
                        </div>
                    </div>
                    <div class="field mt-3">
                        <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">User Limit</label>
                    ...
                    <template #footer>
                        ...
                        <Button type="button" label="Save Coupon" icon="pi pi-save"
                            :style="{ 'background-color': 'darkgreen', border: 'darkgreen' }"
                            @click="editCoupon()"></Button>
                    </template>
                </Dialog>
